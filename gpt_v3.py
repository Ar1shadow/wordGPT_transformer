...
'''
input -> token embedding -> block loop -> output logits (for each position, a distribution over the next token)
block : multi-head self attention + dropout -> add & norm -> feed forward + dropout -> add & norm
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads, d_model):
        ...
        self.Prj = nn.Linear(d_model, d_model)  # final projection after concatenating heads 
        self.dropout = nn.Dropout(dropout)  # dropout for regularization, prevent overfitting
        
        # precompute RoPE cos/sin as buffers so they follow .to(device) automatically
        theta = 1.0 / (10000 ** (torch.arange(0, self.d_k, 2).float() / self.d_k))  # (d_k/2,) 每个纬度的频率不同，偶数维和奇数维共享频率
    ...
    def forward(self, x):
        ...
        # 1 : generate Q K V
        query = self.Q(x)  # (B, T, d_model) vector representation of the queries
        key = self.K(x)  # (B, T, d_model) vector representation of the keys
        value = self.V(x)  # (B, T, d_model) vector representation of the values
        
        
        # 2 :split into Multi-heads
        Q = query.view(Batch_size, Time_steps, num_head, d_k).transpose(1, 2)  # (B, num_head, T, d_k)
        K = key.view(Batch_size, Time_steps, num_head, d_k).transpose(1, 2)  # (B, num_head, T, d_k)
        V = value.view(Batch_size, Time_steps, num_head, d_k).transpose(1, 2)  # (B, num_head, T, d_k)
        ...
        out = self.Prj(out)  # (B, T, d_model)
        
        out = self.dropout(out)  # dropout for regularization, prevent overfitting
        return out
     
    def apply_rope(self, x):
        """
        x: (B, num_head, T, d_k)  对每个位置的向量做旋转变换
...
class GPT(nn.Module):
    def __init__(self):
        ...
        self.block = nn.ModuleList([Block(d_model, num_head, d_model * 4) for _ in range(n_layer)])  # stack 3 blocks
        self.layernorm = nn.LayerNorm(d_model)  # final layer norm before projecting to vocab size
        self.d_k = d_k
    ...
    def forward(self, idx):
        ...
        for block in self.block:
            out = block(out)  # (B, T, d_model)
        out = self.layernorm(out)  # (B, T, d_model)
...
class MLVisualizer:
    ...
    def plot_training(self, history : dict, save_path = None): 
        ...
        best_x, best_y = best + 1, history['val_loss'][best]
        ax1.scatter([best_x], [best_y], color='red', zorder=5, s=40, label='Best')
        # keep label inside axes: offset left if best near right edge
        n = len(epochs)
        dx = 5 if best + 5 < n else -5
        ax1.annotate(f'Best Loss: {best_y:.4f}',
                     xy=(best_x, best_y),
                     xytext=(best_x + dx, best_y + 0.2),
...
if __name__ == '__main__':
    ...
    plt_visualizer.plot_training(history, 'training_history_transformer_v3.png')

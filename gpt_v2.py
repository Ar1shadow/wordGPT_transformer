...
'''
input -> token embedding -> multi-head self attention -> feed forward -> output logits (for each position, a distribution over the next token)
...
class GPT(nn.Module):
    def __init__(self):
        ...
        self.ffwd = FeedForward(d_model, d_model * 4)  # position-wise feed forward network
        self.d_k = d_k
    ...
    def forward(self, idx):
        ...
        # 5: feed forward (give time to think about the connections between the tokens)
        out = self.ffwd(out)  # (B, T, d_model)
...
if __name__ == '__main__':
    ...
    plt_visualizer.plot_training(history, 'training_history_transformer_v2.png')
